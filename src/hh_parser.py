...
class HHParser(BaseParser):
    ...
    async def specify_salary_from(self, salary: int):
        salary_form = self.page.locator(src.parser_selectors.SALARY_FORM)
        await salary_form.fill(str(salary))
        await self.page.keyboard.press("Enter")

    async def select_region(self, region: str):
        region_card = self.page.locator(src.parser_selectors.REGION_CARD)
        region_card_el = await region_card.element_handle()

        all_regions_count = await region_card.locator(src.parser_selectors.FIT_REGIONS).count()
        show_all = region_card.locator(src.parser_selectors.SHOW_ALL_REGIONS_BUTTON)
        ...
        for attempt in range(MAX_ATTEMPTS):
            ...
            if await region_card.locator(src.parser_selectors.FIT_REGIONS).count() > all_regions_count:
                await self.page.wait_for_timeout(1000)
                break
        ...
        await region_card.get_by_label(
            src.parser_selectors.SEARCH_REGION_LABEL
        ).fill(region)
        ...
        await self.page.wait_for_function(
            ...
            arg={
                "root": region_card_el,
                "selector": src.parser_selectors.FIT_REGIONS,
                "prev": all_regions_count
            }
        )

        fit_regions = region_card.locator(src.parser_selectors.FIT_REGIONS)
    ...
    async def parse_card(self, card: Locator) -> Vacancy:
        address = await self.safe_text(
            card.locator(src.parser_selectors.VACANCY_ADDRESS)
        )
        address_additional = await self.safe_text(
            card.locator(src.parser_selectors.VACANCY_ADDRESS_ADDITIONAL)
        )
        ...
        url = await card.locator(src.parser_selectors.VACANCY_URL).get_attribute("href")

        return Vacancy(
            title=await self.safe_text(card.locator(src.parser_selectors.VACANCY_TITLE)),
            salary=await self.safe_text(card.locator(src.parser_selectors.VACANCY_SALARY)),
            experience=await self.safe_text(card.locator(src.parser_selectors.VACANCY_EXPERIENCE)),
    ...
    async def parse(self, query: str, max_pages: int, region: str, salary_from: str):
        ...
        await self.safe_goto(self.page, url=config["BASE_URL_HHRU"])

        logger.info("Entering a query...")
        await self.page.fill(src.parser_selectors.SEARCH_INPUT, query)
        await self.page.keyboard.press("Enter")
        ...
        for page_number in range(max_pages):
            ...
            try:
                await self.page.wait_for_selector(src.parser_selectors.VACANCY_CARD)
            except Exception:
            ...
            cards = self.page.locator(src.parser_selectors.VACANCY_CARD)
            ...
            next_page_button = self.page.locator(src.parser_selectors.NEXT_PAGE_BUTTON)
            if await next_page_button.count() > 0:
                await next_page_button.click()
                continue

            page_buttons = self.page.locator(src.parser_selectors.PAGER_PAGE)
            if await page_buttons.count() == 0:
                logger.info("NO pagination controls found – ending pagination")
                break

            current_button = self.page.locator(
                f"{src.parser_selectors.PAGER_PAGE}[aria-current='true']"
            )
